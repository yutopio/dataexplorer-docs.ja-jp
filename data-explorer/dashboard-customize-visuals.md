...
|**X 軸**     |    X 軸のプロパティをカスタマイズできます。 <li>**ラベル** :カスタム ラベルのテキスト。 </li>     | 横棒、縦棒、面、折れ線、散布図、異常、時間グラフ|
|**Render links (リンクのレンダリング)**     |    テーブル内の "https://" で開始するリンクをクリック可能に切り替えます。 <li>**Apply on columns (適用する列)** :URL を含む列を選択します。 </li>     | テーブル|
|**レイアウト**     |    複数統計のビジュアルのレイアウト構成を選択します。 <li>**Apply on columns (適用する列)** :URL を含む列を選択します。 </li>     | 複数統計|
