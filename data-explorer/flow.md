...
> [!Note]
> アプリケーションが [AAD アプリケーション](kusto/management/access-control/how-to-provision-aad-app.md)であり、クラスターでクエリを実行する権限が与えられていることを確認します。
...
[制御コマンドの実行] を使用して結果のアクションを視覚化し、[制御コマンド](kusto/management/index.md)を実行します。
...
> [!Note]
> クエリがドットで始まっている場合には (つまりそれは[制御コマンド](kusto/management/index.md))、[[制御コマンドを実行して結果を視覚化する](#run-control-command-and-visualize-results)] のアクションを使用します。
...
詳細については、「[クエリのベストプラクティス](kusto/query/best-practices.md)」を参照してください。

## <a name="limitations"></a>制限事項

* クライアントに返される結果は、500,000レコードが上限です。 これらのレコードのメモリの合計は 64 MB を超えることはできず、実行時間の上限は 7 分間です。
* このコネクタは、[フォーク](kusto/query/forkoperator.md)および[ファセット](kusto/query/facetoperator.md)演算子をサポートしていません。
* フローの実行には、Microsoft Edge と Chrome が最適です。

## <a name="next-steps"></a>次のステップ

[Microsoft Azure Explorer Logic App コネクタ](kusto/tools/logicapps.md)について説明します。これは、自動的に Kusto クエリとコマンドを予定タスクやトリガーされたタスクの一部として実行できる、もう 1 つの方法です。
