...
* .NET v2 API に切り替えると、この問題が発生した場合に Kusto チームに通知されるので、ストリーミングクライアントの優先度を上げることができます。
...
これらの制限の削除を検討している場合は、まず、実際に値を取得するかどうかを判断します。 特に、結果の切り捨て制限を削除することは、Kusto からに一括データを移動することを意味します。
結果の切り捨て制限を削除するには、エクスポートの目的でコマンドを使用するか、 `.export` 後で集計を行う場合は、Kusto を使用して集計することを検討してください。
