...
* 連続エクスポートの各繰り返しでエクスポートされるファイルの数は、外部テーブルのパーティション分割方法によって異なります。 詳細については、「 [export to external table command](export-data-to-an-external-table.md)」の「notes」セクションを参照してください。 連続エクスポートの反復は、常に*新しい*ファイルに書き込み、既存のファイルには追加されません。 その結果、エクスポートされたファイルの数も、連続エクスポートが実行される頻度 (パラメーター) によって決まり `intervalBetweenRuns` ます。
